...
with tab1:
   st.header("2021 Accident Information", divider = "red")

   st.subheader("Top 5 types of accidents")
   df = pd.read_csv(r'E:/studyyy/year4/senior pj/test/web_test/event2021_withd.csv',encoding='ISO-8859-1')
...
with tab2:
   ...
   accgdf_2021 = gpd.read_file('E:/studyyy/year4/senior pj/test/web_test/count_2021.shp')
   accgdf_2021 = accgdf_2021.rename(columns={'NUMPOINTS': 'acc_count'})
   new_col = ['dcode', 'dname', 'dname_e','acc_count' , 'AREA','pname', 'geometry']
   accgdf_2021 = accgdf_2021.reindex(columns=new_col)

   # Fillna 
   accgdf_2021['acc_count'].fillna(0, inplace=True)
   ...
   for _, row in accgdf_2021.iterrows():
       folium.Marker(
   ...
   acc_no2021 = process_data("E:/studyyy/year4/senior pj/test/web_test/count_acc_d2021.csv")
   st.dataframe(acc_no2021)
...
with tab3:
   ...
   w = weights.contiguity.Queen.from_dataframe(accgdf_2021)

   # Plotting to visualize spatial weights
   fig, ax = plt.subplots(figsize=(10, 5))
   plot_spatial_weights(w, accgdf_2021, ax=ax)
   #st.pyplot(fig)
   ...
   morans_I, p_value = calculate_and_plot_morans_I(accgdf_2021, w)
   ...
   moran = Moran(accgdf_2021['acc_count'], w)
 ...
 ##########local
   ...
   y_acc_count = accgdf_2021['acc_count']
   ...
   accgdf_2021 = local_classification(accgdf_2021, acc_count_local_moran)

   # อ่านไฟล์ HTML
   with open("E:/studyyy/year4/senior pj/test/web_test/local_moran_map2021.html", "r") as file:
       html_code = file.read()
   ...
   dname_by_moran_class = get_dname_by_moran_class(accgdf_2021)
   # แสดงผลใน Streamlit
